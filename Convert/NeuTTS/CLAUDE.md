...
- **litert-torch installed in two steps.** At v0.9.0 its setup.py pinned `litert-converter==0.1.*` when no such PyPI release existed, so a plain editable install failed. At v0.9.1 (current vendor pin) setup.py wants `litert-converter==0.2.*`, which now exists as a stable release — but the validated path remains: install upstream's `requirements.txt` first (nightly ai-edge wheel set + `--pre`), then `pip install --no-deps -e ../../LiteRT/vendor/litert-torch`.
